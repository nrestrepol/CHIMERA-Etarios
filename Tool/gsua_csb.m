...
for i=1:reps
    ...
    J_test=zeros(1,N);
    parfor (h=1:N,parforArg)
    %for h=1:N
        if inputs>1
            cost=(sum((squeeze(Y_test(h,:,:))'-y_Nom).^2,2)/len)./limit;
            for j=1:inputs
                cost(j)=((2-corr2(y_Nom(j,:),squeeze(Y_test(h,:,j))))*cost(j))^2;
            end
            J_test(h)=sum(cost);
        else
            cost=((sum((Y_test(h,:)-y_Nom).^2,2)/len)./limit).^2;
            J_test(h)=(2-corr2(y_Nom,Y_test(h,:)))*cost;
        end
    end
    ...
    clf
    try
        sup(i)=find(BFM_esc(:,1)<inputs, 1, 'last' );  
